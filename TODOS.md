...
The practical effect is unchanged, so the workaround stands: run `yarn typecheck && yarn test:unit` *inside* the drift job and stamp a loud "❌ Catalog guards failed" section into the PR body when they fail, so the result is visible whether or not anyone approves the gated run. The real fixes are approving the run (Actions tab → **Approve and run**, or `POST /actions/runs/{id}/approve`) or relaxing the repo's require-approval-for-bot-runs setting; a PAT also sidesteps it.
