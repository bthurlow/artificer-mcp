...
- **Endpoint**: `https://fal.run/ideogram/v4/image-to-image`
- **Model ID**: `ideogram/v4/image-to-image`
...
- **`image_size`** (`ImageSize | Enum`, _optional_):
  The output image size. Use 'auto' to match the input image size (capped at ~25MP, up to 8192px per side). Default value: `auto`
  - Default: `"auto"`
...
- **`image_url`** (`string`, _required_):
  ...
  - Examples: "https://storage.googleapis.com/falserverless/model_tests/svd/rocket.png"

- **`strength`** (`float`, _optional_):
  How much to transform the input image. 1.0 ignores the input image entirely; lower values stay closer to it. Default value: `0.8`
  - Default: `0.8`
  - Range: `0` to `1`
...
```bash
curl --request POST \
  --url https://fal.run/ideogram/v4/image-to-image \
...
const result = await fal.subscribe("ideogram/v4/image-to-image", {
  input: {
    prompt: "A red panda perched on a mossy branch in a misty forest at sunrise",
    image_url: "https://storage.googleapis.com/falserverless/model_tests/svd/rocket.png"
  },
...
- [Model Playground](https://fal.ai/models/ideogram/v4/image-to-image)
- [API Documentation](https://fal.ai/models/ideogram/v4/image-to-image/api)
- [OpenAPI Schema](https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=ideogram/v4/image-to-image)
